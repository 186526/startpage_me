...
async function dig(domain, type, short = false) {
    const starttime = (Math.round(performance.now() * 100) / 100 / 1000).toFixed(3);
    let a = dns.doh_json_api(
        domain,
        type = type
    );
    let b = await a;
    if (await b.Status == 0) {
        const endtime = (Math.round(performance.now() * 100) / 100 / 1000).toFixed(3);
        return digoutput(
            domain,
            await b.Answer,
            type,
